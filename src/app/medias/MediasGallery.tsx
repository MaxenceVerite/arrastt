...
export default function MediasGallery({ mediasUrls }: { mediasUrls: any[] }) {
  const [selectedMedia, setSelectedMedia] = useState<any>(null);

  // Parse medias from string URLs or objects {url, caption}
  const medias = mediasUrls.map((item, i) => {
    const url = typeof item === 'string' ? item : item.url;
    const caption = typeof item === 'string' ? "" : item.caption;

    // Basic detection for video extension (mp4, webm)
    ...
    return {
      ...
      title: caption || (isVideo ? "Vidéo Arras TT" : "Photo Arras TT"),
      src: url,
      aspectClass,
    };
  });
  ...
  if (medias.length === 0) {
    return <p className="text-center text-zinc-500 italic mt-8">Aucun média n'a encore été ajouté par l'administration.</p>;
  }
